...
let CalculationService = {
  ...
  createDifferential(
    ...
    results.profiles.forEach((profile) => {
      ...
      // profile.bindings = [];
      // profile.children.forEach(segOrGrp => {
      //   segOrGrp.bindings = []
      //   if(segOrGrp.children){
      //     segOrGrp.children.forEach(element => {
            ...
            if (component) {
              if (context === 'datatype_component') {
                ...
                if (component.data.bindings.length === 0) {
                  ...
                  if (
                    !component.data.bindings[0].data.locations.src
                      .value
                  ) {
                  ...
                  if (
                    !component.data.bindings[0].data.strength.src
                      .value
                  ) {
                  ...
                  if (
                    !component.data.bindings[0].data.valuesets.src
                      .value
                  ) {
                  ...
                  if (
                    ...
                    (!component.data.bindings[0].data.codes ||
                      !component.data.bindings[0].data.codes.src
                        .value)
                  ) {
          ...
          if (bindingDifferential) {
            if (bindingDifferential.data.status === 'added') {
              ...
              const derivedLocations =
                ValuesetService.extractLocations(
                  derivedBinding.locations
                );
                  ...
                  if (srcVs) {
                    ...
                    if (
                      ...
                      if (comparedCodes.changed) {
                        differential.changed = true;
                        differential.data.changed = true;
                        differential.data.changeTypes.push(
                          'valueset'
                        );
                        bindingDifferential.changed = true;
                        bindingDifferential.data.changed = true;
            ...
            newBindingDifferential.data.locations.derived[
              derivedIgId
            ] = {
              value: ValuesetService.extractLocations(
                derivedBinding.locations
            ...
            newBindingDifferential.data.valuesets.derived[
              derivedIgId
            ] = {
              value: ValuesetService.extractVSWithVersion(
                derivedBinding.valuesets,
      ...
      derivedFields.forEach((derivedField) => {
        ...
        if (fieldDifferential) {
          if (fieldDifferential.data.added) {
            ...
            // TODO: check if needed for slicing
          } else {
      ...
      segmentDifferential.children.forEach((differential) => {
        ...
        if (derivedField) {
          if (differential.data.added) {
            ...
            differential.changed = true;
            differential.data.changed = true;
            differential.data.changeTypes.push('field');
  ...
  compareFieldsData(
    ...
    if (configuration.datatype) {
      if (fieldDifferential.data.slicing && derivedField.slicing) {
        ...
        if (
          fieldDifferential.data.slicing.type !==
          derivedField.slicing.type
        ) {
          // change in slicing type
        } else {
          // same slicing, check slice
          derivedField.slicing.slice.forEach((slice) => {
            if (derivedField.slicing.type === 'OCCURRENCE') {
              // OCCURRENCE slicing
              let existingSlice =
                fieldDifferential.data.slicing.slice.find(
                  (s) =>
                    s.data.occurence.src.value === slice.occurence
                );
              if (existingSlice) {
                // compare slice
                if (
                  existingSlice.data.flavor.src.value !== slice.flavor
                ) {
                  existingSlice.data.flavor.derived[derivedIgId] = {
                    value: slice.flavor,
                    status: 'changed',
                  };
                ...
                if (
                  ...
                  existingSlice.data.comment.derived[derivedIgId] = {
                    value: slice.comment,
                    status: 'changed',
                  };
                  existingSlice.data.occurence.derived[derivedIgId] =
                ...
                newSlice.data.occurence.derived[derivedIgId] = {
                  value: slice.occurence,
                  status: 'added',
                };
                newSlice.data.flavor.derived[derivedIgId] = {
                  value: slice.flavor,
                  status: 'added',
                };
                newSlice.data.comment.derived[derivedIgId] = {
                  value: slice.comment,
                  status: 'added',
                };
                fieldDifferential.data.slicing.slice.push(newSlice);
                fieldDifferential.data.slicing.changed = true;
                segmentDifferential.changed = true;
                segmentDifferential.data.changed = true;
                segmentDifferential.data.changeTypes.push('slicing');
              ...
              let existingSlice =
                fieldDifferential.data.slicing.slice.find(
                  (s) =>
                    s.data.assertion.src.value === slice.assertion
                );
              if (existingSlice) {
                // compare slice
                if (
                ...
                newSlice.data.assertion.derived[derivedIgId] = {
                  value: slice.assertion,
                  status: 'added',
                };
                newSlice.data.flavor.derived[derivedIgId] = {
        ...
        let diff = {
          ...
          name: derivedField.slicing.name,
          element: derivedField.slicing.element,
          slice: derivedField.slicing.slice.map((s) => {
            let result = {
              data: {
                assertion: {
                  src: {
            ...
            result.data.assertion.derived[derivedIgId] = {
              value: s.assertion,
              status: 'added',
            };
            result.data.occurence.derived[derivedIgId] = {
              value: s.occurence,
              status: 'added',
            };
            result.data.flavor.derived[derivedIgId] = {
              value: s.flavor,
              status: 'added',
            };
            result.data.comment.derived[derivedIgId] = {
              value: s.comment,
              status: 'added',
            };
            return result;
        ...
        fieldDifferential.data.changed = true;
        fieldDifferential.data.changeTypes.push('slicing');
        fieldDifferential.data.slicing = diff;
      }
      if (
      ...
      if (configuration.valueset) {
        ...
        fieldDifferential.bindings = null;
      }
